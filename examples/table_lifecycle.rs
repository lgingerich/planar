...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Table created: {}.{}", table_ident.namespace, table_ident.name);
    ...
    println!("   Total records: {}", view_after_txn1.files.iter().map(|f| f.record_count).sum::<i64>());
    println!();
    ...
    println!("   Total records: {}", view_after_txn2.files.iter().map(|f| f.record_count).sum::<i64>());
    println!();
    ...
    let historical_view = table_handle
        .read_at(view_after_txn1.transaction_id)
        .await?;
    
    println!("✅ Read table at transaction {}", historical_view.transaction_id);
    ...
    let delta = table_handle
        .diff(view_after_txn1.transaction_id, view_after_txn2.transaction_id)
        .await?;
    ...
    println!("   Total records: {}", view_after_txn3.files.iter().map(|f| f.record_count).sum::<i64>());
    println!();
    ...
    println!("   Total records: {}", final_view.files.iter().map(|f| f.record_count).sum::<i64>());
    println!("   Total size: {} bytes", final_view.files.iter().map(|f| f.file_size_bytes).sum::<i64>());
